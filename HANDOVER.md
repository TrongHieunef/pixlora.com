...
> Sanity ở dự án này **chỉ quản lý SẢN PHẨM** (cho Shop + trang chi tiết). Trang
> chủ (hero, album, featured, testimonials, packages) là **nội dung tĩnh**, sửa
> trong code (xem mục 9) — không qua Sanity.
...
```
Bấm **Create → Sản phẩm** → thêm vài sản phẩm (nhớ up **Ảnh chính**, **Gallery**
nhiều ảnh, và **YouTube ID** nếu có video) để test.
...
**Phạm vi của Sanity:** chỉ dữ liệu **sản phẩm** (Shop + trang chi tiết). Trang
chủ là tĩnh (mục 9). Nếu để trống `projectId` hoặc CMS chưa có sản phẩm nào →
Shop tự dùng `data/products.json`, web vẫn chạy bình thường.
...
2. [`js/shop.js`](js/shop.js) có `loadProducts()` lấy sản phẩm theo thứ tự ưu
   tiên: **Sanity → `/api/products` (nếu chạy backend) → `data/products.json` →
   bản nhúng trong `js/data.js`**. Cả Shop lẫn trang chi tiết đều dùng hàm này.
3. Ảnh trả về là URL trên CDN Sanity; web tự thêm `?w=...&h=...&fit=crop` để
   resize (hàm `sanityImg`). `specs` dạng mảng `{key,value}` được tự gộp thành
   object.
